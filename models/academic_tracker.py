...
class AcademicTracker(models.Model):
    ...
    def retrieve_dashboard_data(self, start_date=False, end_date=False, manager_id=False):
        ...
        academic_domains = actions_common.get_academic_domains(self, department_obj=department_obj,
        ...
        dashboard_data['quantitatives'] = actions_common.get_raw_quantitative_data(self, employees, start_date,
                                                                                   end_date)
        print(dashboard_data['qualitatives'], 'qual')
        print(dashboard_data['quantitatives'], 'quan')

        for employee in employees:
            total_completed = 0
            employee_academic_domains = actions_common.get_academic_domains(self, department_obj=department_obj,
                                                                            start_date=start_date, end_date=end_date,
                                                                            manager=False, managers=False,
                                                                            employee_user_ids=[employee.user_id.id])
            employee_academic_counts = actions_common.get_academic_counts(self, employee_academic_domains)
            print(employee_academic_counts, 'countsss')
            ...
            print(employee, 'vvvv')
            quantitative_overall_objs = self.env['employee.quantitative.performance'].sudo().search(
                [('employee', '=', employee.id)])

            # print("scoreEE", self.env['logic.common.task.performance'].sudo().search([('employee','=',employee.id)]))
            a = self.env['logic.common.task.performance'].sudo().search([('employee', '=', employee.id)])
            print('aa', a[0].qualitative_average_rating, a[0].employee.name, a[0].misc_average_rating)
            print('quant', quantitative_overall_objs.overall_average, quantitative_overall_objs.employee.name, )
            if a[0].employee.id == quantitative_overall_objs.employee.id:
                score = round(a[0].qualitative_average_rating + quantitative_overall_objs.overall_average + a[0].misc_average_rating, 3)
            else:
                score = 0
            values = {
                'employee': employee.id,
            ...
            acad_coord_perf_obj = self.env['academic.coordinator.performance'].sudo().search(
                [('employee', '=', employee.id)])
        ...
        academic_coord_perfs = self.env['academic.coordinator.performance'].sudo().search(
    ...
    def get_batchwise_coordinator_rating(self, employee, batch):
        student_feedbacks = self.env['student.feedback'].sudo().search(
            [('coordinator_id', '=', employee.user_id.id), ('batch_id', '=', batch.id)])
    ...
    def retrieve_employee_academic_data(self, employee_id, start_date=False, end_date=False):
        ...
        if not start_date or not end_date:
            upaya_objs = self.env['upaya.form'].sudo().search(
                [('state', '=', 'complete'), ('coordinator_id', '!=', False),
                 ('coordinator_id', '=', employee.user_id.id)])
            yes_plus_objs = self.env['yes_plus.logic'].sudo().search(
                [('state', '=', 'complete'), ('coordinator_id', '!=', False),
                 ('coordinator_id', '=', employee.user_id.id)])
            one_to_one_objs = self.env['one_to_one.meeting'].sudo().search(
                [('coordinator_id', '!=', False), ('coordinator_id', '=', employee.user_id.id)])
            exam_objs = self.env['exam.details'].sudo().search(
                [('coordinator', '!=', False), ('coordinator', '=', employee.user_id.id)])
        else:
            start_date, end_date = actions_common.get_date_obj_from_string(start_date, end_date)

            upaya_objs = self.env['upaya.form'].sudo().search(
                [('state', '=', 'complete'), ('coordinator_id', '!=', False),
                 ('coordinator_id', '=', employee.user_id.id), ('date', '>=', start_date), ('date', '<=', end_date)])
            yes_plus_objs = self.env['yes_plus.logic'].sudo().search(
                [('state', '=', 'complete'), ('coordinator_id', '!=', False),
                 ('coordinator_id', '=', employee.user_id.id), ('date_one', '>=', start_date),
                 ('date_one', '<=', end_date)])
            one_to_one_objs = self.env['one_to_one.meeting'].sudo().search(
                [('coordinator_id', '!=', False), ('coordinator_id', '=', employee.user_id.id),
                 ('added_date', '>=', start_date), ('added_date', '<=', end_date)])
            exam_objs = self.env['exam.details'].sudo().search(
                [('coordinator', '!=', False), ('coordinator', '=', employee.user_id.id), ('date', '>=', start_date),
                 ('date', '<=', end_date)])
        ...
        for i in range(longest_len):
            ...
            try:
                current_row_data['one_to_one_name'] = one_to_one_objs[i].student_name.name + " - " + dict(
                    one_to_one_objs[i]._fields['reason_for_meeting'].selection).get(
                    one_to_one_objs[i].reason_for_meeting)
            except:
